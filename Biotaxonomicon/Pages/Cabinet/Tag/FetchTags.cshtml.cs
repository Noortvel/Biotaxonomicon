...
{
    public class FetchTagsModel : PageModel
    {
        public IActionResult OnGet(string tagStr)
        {
            ...
            if (tagStr != null)
            {
                var sqlCmd = $"select id,latin_name, russian_name from tags where lower(latin_name) like '%{tagStr.ToLower()}%' or lower(russian_name) like '%{tagStr.ToLower()}%'";
